...
stoplist = set(stopwords.words("english")) #stopwords nltk corpus
...
for line in file(arg_file):
    words =line.split()
    for w in words:
        if word_count[w]>=5 and (w not in stoplist or w in protected_words): #stopwords
        #if (word_count[w]>=5 and word_count[w]<100000) or w in protected_words): #histogram
            f.write(w+" ")
            total_words+=1
